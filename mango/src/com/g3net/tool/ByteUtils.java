...
//						+ fromHexDigit(sr.read());
	 ...
	 * @return
	 */
	public static byte[] fromOctalAscii(String s){
		try {
			int len = s.length();
			if ((len % 8) != 0)
				throw new NumberFormatException(
...
//						+ fromHexDigit(sr.read());
	...
	public static byte[] getDataOutputStreamBytes(String s) {
	...
	public static byte[] getDataOutputStreamBytes(int i) {
		ByteArrayOutputStream byteOut = new ByteArrayOutputStream();// 缓存
		DataOutputStream dos = new DataOutputStream(byteOut);
		try {
			dos.writeInt(i);
	...
	public static byte[] getDataOutputStreamBytes(short i) {
		ByteArrayOutputStream byteOut = new ByteArrayOutputStream();// 缓存
		DataOutputStream dos = new DataOutputStream(byteOut);
		try {
			dos.writeShort(i);
	...
	public static byte[] fromBinaryAscii(String s){
...
//		log.info(toDecimalAscii(new byte[] { 45, -125, 67 }, " "));
//		log.info(toBinaryAscii(new byte[] { 45, -125, 67 }, " "));
//		byte[] bs=convertDouble2Bytes(-5499.13f);
//		log.info(convertBytes2Double(bs));
//		//ConcurrentSkipListMap,
//		 bs=convertChar2Bytes('0');
//		log.info(convertBytes2Char(bs));
//		log.info(ArrayUtils.toString(fromHexAscii(toHexAscii(new byte[] { 45, -125, 67 }," ")," "),","));
//	
		log.info(toBinaryAscii(new byte[] { 45, -125, 67 }," "));
		log.info(ArrayUtils.toString(fromBinaryAscii(toBinaryAscii(new byte[] { 45, -125, 67 }," ")," "),","));
		String s=toOctalAscii(new byte[] { 45, -125, 67 }," ");
		log.info(s);
		s="055 603 77";
		log.info(ByteUtils.toOctalAscii(fromRadixAscii(s, " ", 8)," "));
		String ss=toDecimalAscii(new byte[] { 45, -125, (byte)245 }, ",");
		log.info(ss);
		 ss="45,-12,-11";
		byte[] bss=ByteUtils.fromDecimalAscii(ss,",");
		log.info(ArrayUtils.toString(bss));
		log.info(ByteUtils.toDecimalAscii(bss,","));
		//Byte.decode("-0x10");
		//Scanner sc
		//log.info(format(new byte[]{1, -125, 67}));
		//SocketChannel 
		//log.info(parseByte("12",16));
		
		log.info(ArrayUtils.toString("中".getBytes("utf-16le"),""));
		byte[] bss2="中".getBytes("utf-16be");
		log.info(ByteUtils.convertFromUTF16beBytes2Char(bss2));
