...
public class OneThreadHttpClient {
	private static Logger log = Logger.getLogger(OneThreadHttpClient.class);
	...
	private static ThreadLocal<DefaultHttpClient> httpClient = new ThreadLocal<DefaultHttpClient>(){
		protected DefaultHttpClient initialValue() {
			DefaultHttpClient dh = getHttpClient(connecttion_timeout, so_timeout);
			return dh;
	...
	public OneThreadHttpClient(int connecttion_timeout, int so_timeout) {
	...
	public void shutdown() {
		if (httpClient != null) {
			httpClient.get().getConnectionManager().shutdown();
		}
	}
